...
# Streamlit page configuration
st.set_page_config(page_title="Capture Platform", layout="wide", 
                   page_icon="📝" )
...
def process_files_tab():
    ...
    api_choice = st.selectbox(
        ...
        options=[
            # ("process/pdf2json", "Model-1 PDF to JSON - Pytesseract(OCR) + Gemini LLM [API Used: process/pdf2json]"),
            ("process/OCR+bytes", "Model-1 OCR_Bytes - Images in Bytes given to LLM [API Used: process/OCR+bytes]"),
            ("process/HandwrittenCRM", "Model-2 HandwrittenCRM - For Handwritten PDFs [API Used: process/HandwrittenCRM]"),
            ("upload", "Model-3 Visdeal & Apicem Invoice Extraction (Pytesseract + Gemini LLM) [API Used: process/upload]")
        ],
        format_func=lambda x: x[1]
    )
