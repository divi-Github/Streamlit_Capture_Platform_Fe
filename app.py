...
# # # #     ### Introduction
# # # #     The ARGUS System processes files (PDFs, images, etc.) to extract data using Tesseract OCR and Gemini-2.5-Flash Vision. Below are the steps to use the system and details of the backend processes.
...
# # # #        - Files are processed automatically by the pipeline.
# # # #     5. **What is a Dataset?** 
# # # #        - The Gemini model processes files based on the model prompt and example schema.
# # # #        - The schema can be empty; Gemini will infer a structure if none is provided.

# # # #     #### 2. Exploring Data
# # # #     1. **Navigate to the "🔎 Explore Data" tab**.
# # # #     2. **Fetch Data**:
# # # #        - Data is fetched from PostgreSQL and displayed in a table.
# # # #     3. **Interact with Data**:
# # # #        - Use checkboxes to select files for actions (delete, reprocess).
# # # #        - View raw files and extracted JSON for a single selected file.
# # # #     4. **Provide Feedback**:
# # # #        - Rate extraction quality and add comments for selected files.

# # # #     #### 3. Adding New Dataset
# # # #     1. **Navigate to the "🧠 Process Files" tab**.
# # # #     2. **Add New Dataset**:
# # # #        - Enter a dataset name, model prompt, and example schema.
# # # #        - Click 'Add New Dataset' to save to PostgreSQL.

# # # #     #### 4. Additional Notes
# # # #     - **Reprocessing**: Reprocess failed files from the "Explore Data" tab.

# # # #     ### Backend Processes
# # # #     1. **File Upload**:
# # # #        - Files are saved to `./uploads/<dataset_name>`.
# # # #     2. **Processing Pipeline**:
# # # #        - Tesseract performs OCR to extract text.
# # # #        - Gemini-2.5-Flash Vision processes OCR output to extract structured data.
# # # #     3. **Data Storage**:
# # # #        - Results and metadata are stored in a local PostgreSQL database.
# # # #     4. **Data Retrieval**:
# # # #        - The "Explore Data" tab fetches data from PostgreSQL for display.
...
# # # #         options=[
# # # #             ("process/pdf2json", "Model-1 PDF to JSON - Pyteseract(OCR) + Gemini LLM "
            
# # # #             "[API Used : process/pdf2json]"),
            
# # # #             ("process/OCR+bytes", "Model-2 OCR + Bytes - Images in Bytes to LLM "
            
# # # #             "[API Used: process/OCR+bytes]")
# # # #         ],
...
# # # # # def main():
# # # # #     st.set_page_config(page_title="ARGUS System Frontend", layout="wide")

# # # # #     tabs = ["Instructions", "Process Files"]
# # # # #     active_tab = st.sidebar.radio("Navigation", tabs)

# # # # #     if active_tab == "Instructions":
# # # # #         instructions_tab()
# # # # #     elif active_tab == "Process Files":
# # # # #         process_files_tab()
...
# # # API_BASE_URL = "http://localhost:8000/data"  # Matches /data prefix
...
# # #     ### Introduction
# # #     The Intelligent Data Extraction (JSON) and Model Comparison for PDFs processes files (PDFs) to extract data using Tesseract OCR and Gemini-2.5-Flash Vision. 
...
# # #     1. **Navigate to the "🔎 Explore Data" tab**.
# # #     2. **Fetch Data**:
# # #        - Data is fetched from PostgreSQL via the backend API and displayed in a table.
# # #     3. **Interact with Data**:
# # #        - Select a file from the dropdown on the left to view its extracted JSON data on the right.
...
# # #        - Results and metadata are stored in a local PostgreSQL database.
# # #     4. **Data Retrieval**:
# # #        - The "Explore Data" tab fetches data via the backend API.
...
# # #         endpoint = f"/{api_choice[0]}"  # Already includes /process
# # #         url = "http://localhost:8000" + endpoint  # Use base URL without /data for process endpoints
...
# # #     if not documents:
# # #         return  # Warning already displayed in fetch_documents
...
# # #             format_func=lambda x: x,  # id is the filename
# # #             key="explore_select"
# # #         )
...
# # #             doc = df[df['id'] == selected_doc_id].iloc[0]
# # #             st.write(f"**ID (Filename)**: {doc['id']}")
# # #             if 'data' in doc and isinstance(doc['data'], dict):
# # #                 if 'properties' in doc['data']:
# # #                     st.write(f"**Request Timestamp**: {doc['data'].get('properties', {}).get('request_timestamp', 'N/A')}")
# # #                     st.write(f"**Number of Pages**: {doc['data'].get('properties', {}).get('num_pages', 'N/A')}")
# # #                     st.write(f"**Total Time (Seconds)**: {doc['data'].get('properties', {}).get('total_time_seconds', 'N/A')}")
# # #                 if 'extracted_data' in doc['data']:
# # #                     st.subheader("Extracted JSON")
# # #                     st.json(doc['data']['extracted_data'])
# # #                 else:
...
# # #         doc1_source = st.radio("Source for First JSON", ["Database", "Upload"], key="doc1_source")
# # #         if doc1_source == "Database":
...
# # #             json1 = doc1['data']['extracted_data'] if doc1 is not None and 'data' in doc1 and isinstance(doc1['data'], dict) and 'extracted_data' in doc1['data'] else None
# # #             if json1:
...
# # #         doc2_source = st.radio("Source for Second JSON", ["Database", "Upload"], key="doc2_source")
# # #         if doc2_source == "Database":
...
# # #             json2 = doc2['data']['extracted_data'] if doc2 is not None and 'data' in doc2 and isinstance(doc2['data'], dict) and 'extracted_data' in doc2['data'] else None
# # #             if json2:
...
# #             if 'data' in doc and isinstance(doc['data'], dict) and 'extracted_data' in doc['data']:
# #                 with st.expander("View Extracted JSON", expanded=False):
# #                     st.code(json.dumps(doc['data']['extracted_data'], indent=2), language="json")
# #                     # Add download button for JSON
# #                     json_str = json.dumps(doc['data']['extracted_data'], indent=2)
# #                     st.download_button(
# #                         label="Download JSON",
# #                         data=json_str,
...
#     else:
#         elements.append(Paragraph("No OCR output available.", styles['Normal']))
#     elements.append(Spacer(1, 12))
...
#     else:
#         elements.append(Paragraph("No extracted JSON data available.", styles['Normal']))
#     elements.append(Spacer(1, 12))
...
MODEL3_API_BASE_URL = "https://c7ff6def311f.ngrok-free.app" 
