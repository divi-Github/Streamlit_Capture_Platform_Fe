...
# MODEL3_API_BASE_URL = "https://2bb6818811cf.ngrok-free.app"  # Update with your actual ngrok URL
MODEL3_API_BASE_URL = "https://c7ff6def311f.ngrok-free.app"  # Update with your actual ngrok URL
...
def enhanced_pie_chart(comparison):
    df_chart = pd.DataFrame({
        'Status': ['Matched', 'Mismatched'],
        'Count': [comparison['matched_keys'], comparison['mismatched_keys']],
        'Percentage': [
            (comparison['matched_keys'] / comparison['total_keys'] * 100) if comparison['total_keys'] else 0,
            (comparison['mismatched_keys'] / comparison['total_keys'] * 100) if comparison['total_keys'] else 0
        ]
    })
    fig = px.pie(
        df_chart, 
        ...
        values='Count', 
        color='Status',
        color_discrete_map={'Matched': '#00CC96', 'Mismatched': '#EF553B'},
        title="JSON Key Match Breakdown",
        hover_data=['Percentage']
    )
    fig.update_traces(
        textinfo='percent+label',
        hovertemplate='%{label}: %{value} keys (%{customdata[0]:.2f}%)'
    )
    fig.update_layout(showlegend=True, title_x=0.5)
    st.plotly_chart(fig, use_container_width=True)

def key_mismatch_bar_chart(comparison):
    if not comparison['mismatch_data']:
        st.write("No mismatched keys to visualize.")
        return
    mismatch_df = pd.DataFrame(comparison['mismatch_data'], columns=["Key", "Expected", "Found"])
    mismatch_df['Key_Category'] = mismatch_df['Key'].apply(lambda x: 'Nested' if '.' in x else 'Top-Level')
    category_counts = mismatch_df['Key_Category'].value_counts().reset_index()
    category_counts.columns = ['Key_Category', 'Count']
    ...
    fig = px.bar(
        ...
        y='Count', 
        color='Key_Category', 
        title="Mismatches by Key Type",
        color_discrete_map={'Nested': '#FF6B6B', 'Top-Level': '#4ECDC4'}
    )
    fig.update_layout(xaxis_title="Key Type", yaxis_title="Number of Mismatches")
    st.plotly_chart(fig, use_container_width=True)

def similarity_heatmap(json1, json2):
    flat_json1 = flatten_json(json1)
    flat_json2 = flatten_json(json2)
    common_keys = sorted(set(flat_json1.keys()) & set(flat_json2.keys()))
    if not common_keys:
        st.write("No common keys to compare.")
        return
    
    similarity_matrix = np.zeros((len(common_keys), 1))
    for i, key in enumerate(common_keys):
        val1, val2 = str(flat_json1[key]), str(flat_json2[key])
        similarity_matrix[i, 0] = 1 - (levenshtein_distance(val1, val2) / max(len(val1), len(val2))) if max(len(val1), len(val2)) > 0 else 1

    fig, ax = plt.subplots(figsize=(10, len(common_keys) * 0.4))
    sns.heatmap(
        similarity_matrix,
        yticklabels=common_keys,
    ...
    plt.title("Key Value Similarity Heatmap")
    plt.tight_layout()
    st.pyplot(fig)
...
def instructions_tab():
    ...
    3. **Visualize**:
       - See a pie chart of matched vs. mismatched keys, a bar chart of mismatch types, and a heatmap of key value similarities.
...
def json_accuracy_tab():
    ...
    if json1 and json2:
        ...
        enhanced_pie_chart(comparison)

        st.subheader("Mismatches by Key Type")
        key_mismatch_bar_chart(comparison)

        st.subheader("Key Value Similarity")
        similarity_heatmap(json1, json2)
